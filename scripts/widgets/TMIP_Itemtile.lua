...
local function FindAtlas(str)
    ...
    -- if AllRecipes[str] then
    --     local atlas=AllRecipes[str].atlas
    --     local image=AllRecipes[str].image
...
local item_list_bm={
    --牛装饰品
    yotb_beefalo_doll_=function (prefabsname)
        return string.match(prefabsname,"yotb_(.+)")
    end,
    --如果是大理石雕像则移除大理石的后缀
    _marble=function (prefabsname)
        return RemoveSuffix(prefabsname)
    end,
}
local list_to={
    playbill_the_doll="playbill",--附魔玩偶
    cursed_monkey_token="cursed_beads1",--诅咒
    --知识饮水机
    archive_lockbox_dispencer="archive_knowledge_dispensary",
    chessjunk="chessjunk",
    archive_rune_statue="archive_runes",
    --远古锅
    archive_cookpot="cookpot_archive",
    --远古雕像
    ruins_statue_="statue_ruins",
    --另一种远古雕像
    atrium_statue="atrium_statue",
    --远古方尖碑
    atrium_idol="atrium_overgrowth",
    --月石雕像
    archive_moon_statue="archive_moon_statue1",
}
function ItemTile:GetAsset(find)
        ...
        --碟子啥的
        local ruins_list={
            "_bowl","_chair","_chipbowl","_plate","_table","_vase"
        }
        if string.find(prefabsname,"ruins_")then
            for _,k in pairs(ruins_list)do
                if string.find(prefabsname,k) then
                    prefabsname="ruinsrelic"..k--"relic"
                    break;
                end
            end
        end
        ...
        -- if string.find(prefabsname,"cursed_monkey_token") or string.find(prefabsname,"monkey") or string.find(prefabsname,"cursed") then
        --     print("对不对",prefabsname)
        -- end
...
function ItemTile:DescriptionInit()
        ...
        if prefabsname and prefabsname ~= "" then
            ...
            elseif string.find(prefabsname, "_") then
                ...
                if string.find(prefabsname,"_BLUEPRINT") then
                    local tmp_str=string.match(prefabsname,"(.+)_BLUEPRINT")
                    local str=STRINGS.NAMES[tmp_str]
                    if str then
                        str=str.."蓝图"
                        return str
                    end
